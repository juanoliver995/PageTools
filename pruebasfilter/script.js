let listTools = [];
let toolsWithButtonActive = []
let buttonsIds = []
let toolsWithButtonDesactive = []
    ...
    pushable.addEventListener("click", function (e) {
        ...
        if (childPushable.classList.contains("tool-btn-active")) {
            ...
            if (buttonsIds.length == 0) {
                ...
                buttonsIds = buttonsIds.filter(id => id !== idButton);
                arrrayFilteredHidden(buttonsIds)
            }
        }
...
function arrayFilteredTools(tools) {
    toolsWithButtonActive = []
    tools.forEach(id => {
        listTools.filter(tool => {
            tool.Categories.forEach(category => {
                if (category === id && !toolsWithButtonActive.includes(tool)) {
                    toolsWithButtonActive.push(tool)
                } else {
    ...
    visibleCards(toolsWithButtonActive)
}

function arrrayFilteredHidden(tools) {
    toolsWithButtonDesactive = []
    tools.forEach(id => {
        toolsWithButtonActive.filter(tool => {
            tool.Categories.forEach(category => {
                if (category === id) {
                    toolsWithButtonDesactive.push(tool)
                } else {
                    hiddenCardsTools(tool)
                }
            })
    ...
    toolsWithButtonActive = toolsWithButtonDesactive
    visibleCards(toolsWithButtonActive)
}
